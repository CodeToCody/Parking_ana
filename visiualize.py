...
fig.suptitle('2024年每週一到週日每小時平均停車數量與標準差', fontsize=16)

for i, day in enumerate(range(7)):  # 0=週一, 6=週日
    day_data = weekly_hourly_stats[weekly_hourly_stats['星期'] == day]
    axes[i].plot(day_data['小時'], day_data['mean'], label='平均停車數量', color='blue')
    axes[i].fill_between(day_data['小時'], 
                         day_data['mean'] - day_data['std'], 
                         day_data['mean'] + day_data['std'], 
                         color='blue', alpha=0.2, label='標準差範圍')
