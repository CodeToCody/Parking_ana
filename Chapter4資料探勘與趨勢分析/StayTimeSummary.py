# 針對每個月的「工作日」，繪製停車場停留時數的平均分佈圖，並標出熱門區間與短停/常規區間，視覺化月度趨勢。
import pandas as pd
import os
import matplotlib
...
for month, hist in monthly_avg.items():
    try:
        # 計算 0-16 小時數據占比
        month_data = df[df["month"] == month]   # 該月資料
        total_records = len(month_data)         # 該月資料數量
        within_16h_records = len(month_data[month_data["stay_hours"] <= 16])                # 該月停留時數小於 16 小時之資料
        percentage = (within_16h_records / total_records * 100) if total_records > 0 else 0 # 該月停留時數小於 16 小時之資料佔該月資料數量之百分比
        ...
        # xlim()第一個參數: bar 左邊一點點開始
        # 第二個: len(bins) - 1.5 表示設在最後一個 bar 的右邊一點點
